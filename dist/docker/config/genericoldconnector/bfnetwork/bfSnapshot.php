...
require 'bfnetwork/bfPlugin.php';";
    ...
    {
        $this->db->setQuery("SELECT enabled FROM #__extensions WHERE name ='plg_captcha_recaptcha'");
        $enabled = $this->db->loadResult();

        $this->db->setQuery("SELECT params FROM #__extensions WHERE name ='plg_captcha_recaptcha'");
        $keyed = '{"public_key":"","private_key":"","theme":"clean"}' == $this->db->loadResult() ? 0 : 1;

        $configured = ('recaptcha' == $this->config->getCfg('captcha', '') ? 1 : 0);

        $res = $configured + $enabled + $keyed;
